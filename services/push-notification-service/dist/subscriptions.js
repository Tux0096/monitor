...
export async function removePushSubscription(fcmToken) {
...
export async function shouldSendPushAlert(alertKey, cooldownMinutes = 60) {
    const cutoff = sql `now() - (${cooldownMinutes} * interval '1 minute')`;
    const rows = await db
        .select({ alertKey: pushAlertDedup.alertKey })
        .from(pushAlertDedup)
        .where(sql `${pushAlertDedup.alertKey} = ${alertKey} AND ${pushAlertDedup.sentAt} > ${cutoff}`)
        .limit(1);
    return rows.length === 0;
}
export async function markPushAlertSent(alertKey) {
    await db

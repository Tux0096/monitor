...
import {
  checkSlowMetricsAndPush,
  evaluatePerformanceReport,
} from "./alerts/slow-metrics.js";
...
export async function buildApp() {
  ...
  app.post("/push/v1/alerts/evaluate-performance", async (req, reply) => {
    if (!verifyServiceSecret(req.headers["x-monitor-import-secret"])) {
      return reply.status(401).send({ error: "Unauthorized" });
    }

    try {
      const result = await evaluatePerformanceReport(
        req.body as import("./performance-types.js").PerformanceHistoryReport,
      );
